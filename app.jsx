// clubs-app.jsx — NOLA Jazz Tonight club show scheduler
// Discover live jazz happening tonight and tomorrow at New Orleans clubs.
// Swipe right to add a show, left to skip.

const { useState, useEffect, useRef, useMemo, useCallback } = React;

const APP_VERSION = 'clubs-v1';
...
// Helpers
// ─────────────────────────────────────────────────────────────
const VENUE_BY_ID = Object.fromEntries(window.VENUES.map(s => [s.id, s]));
...
function computeLanes(dayBands) {
  ...
  for (const band of sorted) {
    ...
    for (let i = 0; i < laneEnd.length; i++) {
      if (laneStage[i] === band.venue && laneEnd[i] <= s) { best = i; break; }
    }
    // Any open lane
    if (best === -1) {
      for (let i = 0; i < laneEnd.length; i++) {
...
// LocalStorage keys
const LS_KEY      = 'nolajazz.schedule.v1';
const LS_REJECT   = 'nolajazz.rejected.v1';
const LS_DAY      = 'nolajazz.activeDay.v1';
const LS_TOPPICKS = 'nolajazz.topPicks.v1';
...
function BandCard({ band, top, onSwipe, scheduled, autoPlay, onToggleAutoPlay }) {
  ...
  const stage = VENUE_BY_ID[band.venue];
  const day = DAY_BY_ID[band.day];
...
// ─────────────────────────────────────────────────────────────
const LS_INTRO = 'nolajazz.introSeen.v1';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          color: '#F5F1EA',
        }}>NOLA Jazz Tonight</h2>
        <p style={{
          margin: '0 0 24px', textAlign: 'center',
          fontSize: 13, color: 'rgba(245,241,234,0.55)', lineHeight: 1.5,
        }}>Live jazz clubs in New Orleans — tonight &amp; tomorrow</p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 28 }}>
          {[
            ['👆', 'Swipe right to add a show to your night'],
            ['👈', 'Swipe left to skip'],
            ['↩️', 'Tap Undo to bring back the last swipe'],
            ['📅', 'Switch between tonight and tomorrow'],
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <MenuRow icon="ℹ️" label="About NOLA Jazz Tonight" onPress={() => { onClose(); onShowAbout(); }} />
        ...
        <p style={{
          ...
          Live jazz club schedule — New Orleans
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '20px 24px 0' }}>
          ...
          <div style={{ fontSize: 12, color: 'rgba(245,241,234,0.4)', marginBottom: 20, letterSpacing: 0.3 }}>
            Live jazz clubs in New Orleans
          </div>

          <div style={{ fontSize: 14, lineHeight: 1.65, color: 'rgba(245,241,234,0.8)', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <p style={{ margin: 0 }}>
              New Orleans has live jazz happening every night of the week — from the legendary clubs on Frenchmen Street to historic rooms in the French Quarter and beyond. This app pulls the night's schedule from the <strong style={{ color: '#F5F1EA' }}>WWOZ Livewire calendar</strong> so you can discover who's playing before you head out.
            </p>
            <p style={{ margin: 0 }}>
              Swipe through the lineup, watch a clip, and add shows to your night. The neighborhood groups help you plan around where you'll be — Frenchmen St alone has a dozen venues within a short walk.
            </p>
            <p style={{ margin: 0 }}>
              Schedule data is sourced from <strong style={{ color: '#F5F1EA' }}>WWOZ 90.7 FM</strong> — New Orleans' community jazz radio station and the authoritative source for live music listings.
            </p>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: '#FB923C', marginBottom: 4 }}>Data source</div>
              <div style={{ fontSize: 13, color: 'rgba(245,241,234,0.7)', lineHeight: 1.5 }}>
                Show listings are pulled from <strong style={{ color: '#F5F1EA' }}>wwoz.org/calendar/livewire-music</strong>. Schedules are updated daily — refresh the page each evening for the latest listings.
              </div>
            </div>
...
function MineBandSheet({ band, conflictingBands, onClose, onRemove, onRemoveConflict, userTopPicks, onTopPickChange }) {
  ...
  const previewStage = VENUE_BY_ID[previewBand.venue];
...
function computeTopPicks(dayBands, userOverrides = null) {
  const stageOrder = Object.fromEntries(window.VENUES.map((s, i) => [s.id, i]));
    ...
    if (cluster.length === 1) {
      ...
      if (userPick) {
        ...
        const sorted = [...cluster].sort((a, b) => {
          ...
          return aC !== bC ? aC - bC : stageOrder[a.venue] - stageOrder[b.venue];
        });
...
function ScheduleView({ scheduled, activeDay, onRemove, userTopPicks, onTopPickChange }) {
  ...
  const activeStages = window.VENUES.filter(s => dayBands.some(b => b.venue === s.id));
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div ref={bodyRef} onScroll={onBodyScroll}
          ...
          <div style={{ minWidth: activeStages.length * colW, height: totalH + 80, position: 'relative' }}>
            ...
            {/* Event blocks */}
            {dayBands.map(b => {
              const stage = VENUE_BY_ID[b.venue];
              const colIdx = stageColIdx[b.venue] ?? 0;
...
// ─────────────────────────────────────────────────────────────
// Club Schedule — neighborhood-grouped venue list
// ─────────────────────────────────────────────────────────────
function GridView({ activeDay, bands, scheduledIds, onAdd, onRemove }) {
  const [previewBand, setPreviewBand] = useState(null);
  const scrollRef = useRef(null);
  const dayBands = useMemo(() => bands.filter(b => b.day === activeDay).sort((a, b) => toMin(a.start) - toMin(b.start)), [bands, activeDay]);

  // Group venues with shows by neighborhood
  const byNeighborhood = useMemo(() => {
    const venuesWithShows = window.VENUES.filter(v => dayBands.some(b => b.venue === v.id));
    const groups = {};
    for (const v of venuesWithShows) {
      const hood = v.neighborhood || 'Other';
      if (!groups[hood]) groups[hood] = [];
      groups[hood].push(v);
    }
    // Sort neighborhoods: Frenchmen St first, then alphabetical
    const hoodOrder = ['Frenchmen St', 'French Quarter', 'Marigny', 'Bywater', 'Uptown', 'Mid-City', 'CBD', 'Warehouse District', 'Garden District', 'St. Roch', 'Other'];
    return Object.entries(groups).sort(([a], [b]) => {
      const ai = hoodOrder.indexOf(a); const bi = hoodOrder.indexOf(b);
  ...
  return (
    <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '0 0 80px' }}>
      {byNeighborhood.map(([hood, venues]) => (
        <div key={hood}>
          {/* Neighborhood header */}
          <div style={{
            padding: '14px 16px 6px',
            fontSize: 11, fontWeight: 800, letterSpacing: 1.2,
            color: 'rgba(245,241,234,0.4)', textTransform: 'uppercase',
            borderBottom: '1px solid rgba(255,255,255,0.06)',
          }}>{hood}</div>

          {venues.map(venue => {
            const venueShows = dayBands.filter(b => b.venue === venue.id);
            return (
              <div key={venue.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                {/* Venue name */}
                <div style={{
                  padding: '10px 16px 4px',
                  fontSize: 13, fontWeight: 700, color: venue.tone,
                  display: 'flex', alignItems: 'center', gap: 6,
                }}>
                  {venue.outdoor && <span style={{ fontSize: 11 }}>☀️</span>}
                  {venue.name}
                </div>
                {/* Shows at this venue */}
                {venueShows.map(band => {
                  const added = scheduledIds.has(band.id);
                  const isNow = isToday && toMin(band.start) <= nowMin && nowMin < toMin(band.end);
                  const isPast = isToday && nowMin >= toMin(band.end);
                  return (
                    <div
                      key={band.id}
                      ...
                      style={{
                        ...
                        background: isNow ? `${venue.tone}18` : 'transparent',
                      }}
                    >
                      {/* Time */}
                      <div style={{
                        minWidth: 52, fontSize: 12,
                        color: isNow ? venue.tone : 'rgba(245,241,234,0.45)',
                        fontVariantNumeric: 'tabular-nums', fontWeight: isNow ? 700 : 400,
                      }}>
                        {fmtTimeShort(band.start)}
                        {isNow && <span style={{ fontSize: 9, marginLeft: 3, verticalAlign: 'middle' }}>●</span>}
                      </div>
                      {/* Artist name */}
                      <div style={{
                        flex: 1, fontSize: 14,
                        color: isPast ? 'rgba(245,241,234,0.5)' : '#F5F1EA',
                        lineHeight: 1.3,
                      }}>{band.name}</div>
                      {/* Add / check button */}
                      <button
                        onPointerDown={e => e.stopPropagation()}
                        onClick={e => { e.stopPropagation(); added ? onRemove(band) : onAdd(band); }}
                        style={{
                          width: 28, height: 28, borderRadius: 14, border: 0,
                          background: added ? venue.tone : 'rgba(255,255,255,0.1)',
                          color: added ? '#fff' : 'rgba(245,241,234,0.5)',
...
function StagePicker({ open, activeDay, onSelect, onClose }) {
  if (!open) return null;
  const dayBands = window.SHOWS.filter(b => b.day === activeDay);
  const stagesWithCounts = window.VENUES.map(s => ({
    ...s,
    count: dayBands.filter(b => b.venue === s.id).length,
  })).filter(s => s.count > 0);
    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: '#F5F1EA' }}>Browse by Venue</div>
          <div style={{ fontSize: 12, color: 'rgba(245,241,234,0.45)', marginTop: 3 }}>See all acts at one club, in time order</div>
        </div>
    ...
    return (
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, color: '#FACC15', textTransform: 'uppercase' }}>Venue</span>
...
function App() {
  ...
  const { deck, slotContext, allChosen } = useMemo(() => {
    const allDay = bands.filter(b => b.day === activeDay && b.venue !== 'children');

    // ── Stage-browse mode ──────────────────────────────────────
    if (browseStage) {
      const stageBands = allDay
        .filter(b => b.venue === browseStage && !scheduledIds.has(b.id) && !rejectedIds.has(b.id))
        .sort((a, b) => toMin(a.start) - toMin(b.start));
      const stage = VENUE_BY_ID[browseStage];
      return {
        deck: stageBands,
  ...
  const handleSwipe = useCallback((band, dir) => {
    if (dir === 'left') {
      ...
      ph('band_skipped', { band_id: band.id, band_name: band.name, stage: band.venue, day: band.day });
    } else {
      setScheduledIds(prev => new Set([...prev, band.id]));
      ph('band_added', { band_id: band.id, band_name: band.name, stage: band.venue, day: band.day, source: 'swipe' });
    }
      ...
      {window.TweaksPanel && (
        <window.TweaksPanel>
          ...
          <window.TweakButton
            ...
            onClick={() => {
              if (confirm('Remove all shows from your schedule?')) {
...
function SearchOverlay({ open, onClose, scheduledIds, rejectedIds, onAdd, onRemove, onPreview }) {
  ...
  const results = q.length < 1 ? [] : window.SHOWS.filter(b =>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {q.length === 0 ? (
          <div style={{ padding: '40px 20px', textAlign: 'center', color: 'rgba(245,241,234,0.35)', fontSize: 14 }}>
            Type to search all {window.SHOWS.length} artists
          </div>
        ) : results.length === 0 ? (
          <div style={{ padding: '40px 20px', textAlign: 'center', color: 'rgba(245,241,234,0.35)', fontSize: 14 }}>
